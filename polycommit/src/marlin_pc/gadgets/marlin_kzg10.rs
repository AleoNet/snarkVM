...
mod tests {
    ...
    fn single_poly_test_template() -> Result<(), SynthesisError> {
        ...
        for (i, test_component) in test_components.iter().enumerate() {
            ...
            let y: Vec<_> = commitments.iter().map(|x| x).collect();
            let mut rng = snarkvm_utilities::test_rng();

            let p = batch_proof.clone().unwrap().clone();
            let x = PC_Bls12_377::batch_check(
                verification_key,
                y,
                query_set,
                evaluations,
            ...
            result
                .enforce_equal(cs.ns(|| format!("enforce_eval_{}", i)), &Boolean::Constant(true))
                .unwrap();
