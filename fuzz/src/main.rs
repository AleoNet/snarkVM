...
use clap::{self, Parser as ClapParser, StructOpt, Subcommand};
...
use snarkvm_fuzz::{CurrentNetwork, fuzz_program, init_vm};
...
fn fuzz(cli: FuzzCli) {
    ...
    let broker_port = cli.broker_port;
    let cores = cli.cores.clone();
    ...
    let mut run_client = |state: Option<_>, mut restarting_mgr, _core_id| {
        ...
        let mut state = state.unwrap_or_else(|| {
            StdState::new(
                ...
                OnDiskCorpus::new(&cli.output).unwrap(),
        ...
        let mut harness = |input: &BytesInput| {
            ...
            if let Ok(s) =  std::str::from_utf8(buf) {
                if let Ok((s, program)) = Program::<CurrentNetwork>::parse(&s) {
...
/*            let mut unstructured = Unstructured::new(buf);
        ...
        if state.corpus().count() < 1 {
            state
                .load_initial_inputs(&mut fuzzer, &mut executor, &mut restarting_mgr, &cli.input)
                .unwrap_or_else(|_| panic!("Failed to load initial corpus at {:?}", &cli.input));
    ...
    match Launcher::builder()
        ...
        .remote_broker_addr(cli.remote_broker_addr)
