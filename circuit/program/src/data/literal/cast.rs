...
impl<A: Aleo> Literal<A> {
    ...
    pub fn cast(&self, to_type: LiteralType) -> Result<Self> {
        match self {
            Self::Address(address) => cast_address_to_type(address, to_type),
    ...
    pub fn cast_lossy(&self, to_type: LiteralType) -> Result<Self> {
        match self {
            Self::Address(address) => cast_lossy_address_to_type(address, to_type),
...
/// Casts an address literal to the given literal type.
fn cast_address_to_type<A: Aleo>(address: &Address<A>, to_type: LiteralType) -> Result<Literal<A>> {
    match to_type {
        LiteralType::Address => Ok(Literal::Address(address.clone())),
        LiteralType::Boolean => Ok(Literal::Boolean(address.cast()?)),
        LiteralType::Field => Ok(Literal::Field(address.cast()?)),
        LiteralType::Group => Ok(Literal::Group(address.cast()?)),
        LiteralType::I8 => Ok(Literal::I8(address.cast()?)),
        LiteralType::I16 => Ok(Literal::I16(address.cast()?)),
        LiteralType::I32 => Ok(Literal::I32(address.cast()?)),
        LiteralType::I64 => Ok(Literal::I64(address.cast()?)),
        LiteralType::I128 => Ok(Literal::I128(address.cast()?)),
        LiteralType::U8 => Ok(Literal::U8(address.cast()?)),
        LiteralType::U16 => Ok(Literal::U16(address.cast()?)),
        LiteralType::U32 => Ok(Literal::U32(address.cast()?)),
        LiteralType::U64 => Ok(Literal::U64(address.cast()?)),
        LiteralType::U128 => Ok(Literal::U128(address.cast()?)),
        LiteralType::Scalar => Ok(Literal::Scalar(address.cast()?)),
        LiteralType::Signature => bail!("Cannot cast an address literal to a signature type."),
        LiteralType::String => bail!("Cannot cast an address literal to a string."),
    }
}

/// Casts an address literal to the given literal type, with lossy truncation.
fn cast_lossy_address_to_type<A: Aleo>(address: &Address<A>, to_type: LiteralType) -> Result<Literal<A>> {
    match to_type {
        LiteralType::Address => Ok(Literal::Address(address.clone())),
        LiteralType::Boolean => Ok(Literal::Boolean(address.cast_lossy()?)),
        LiteralType::Field => Ok(Literal::Field(address.cast_lossy()?)),
        LiteralType::Group => Ok(Literal::Group(address.cast_lossy()?)),
        LiteralType::I8 => Ok(Literal::I8(address.cast_lossy()?)),
        LiteralType::I16 => Ok(Literal::I16(address.cast_lossy()?)),
        LiteralType::I32 => Ok(Literal::I32(address.cast_lossy()?)),
        LiteralType::I64 => Ok(Literal::I64(address.cast_lossy()?)),
        LiteralType::I128 => Ok(Literal::I128(address.cast_lossy()?)),
        LiteralType::U8 => Ok(Literal::U8(address.cast_lossy()?)),
        LiteralType::U16 => Ok(Literal::U16(address.cast_lossy()?)),
        LiteralType::U32 => Ok(Literal::U32(address.cast_lossy()?)),
        LiteralType::U64 => Ok(Literal::U64(address.cast_lossy()?)),
        LiteralType::U128 => Ok(Literal::U128(address.cast_lossy()?)),
        LiteralType::Scalar => Ok(Literal::Scalar(address.cast_lossy()?)),
        LiteralType::Signature => bail!("Cannot cast an address literal to a signature type."),
        LiteralType::String => bail!("Cannot cast an address literal to a string."),
    }
}
