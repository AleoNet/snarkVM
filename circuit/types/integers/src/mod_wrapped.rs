...
impl<E: Environment, I: IntegerType> ModWrapped<Self> for Integer<E, I> {
    type Output = Self;

    #[inline]
    fn mod_wrapped(&self, other: &Integer<E, I>) -> Self::Output {
        match I::is_signed() {
...
impl<E: Environment, I: IntegerType> Metrics<dyn ModWrapped<Integer<E, I>, Output = Integer<E, I>>> for Integer<E, I> {
    type Case = (Mode, Mode);
...
impl<E: Environment, I: IntegerType> OutputMode<dyn ModWrapped<Integer<E, I>, Output = Integer<E, I>>>
    for Integer<E, I>
{
    type Case = (Mode, Mode);
...
mod tests {
    ...
    fn check_mod<I: IntegerType + RefUnwindSafe>(
        ...
        match I::is_signed() {
            true => check_operation_halts(&a, &b, Integer::mod_wrapped),
            false => {
                if second == console::Integer::zero() {
                    match mode_b {
                        Mode::Constant => check_operation_halts(&a, &b, Integer::mod_wrapped),
                        _ => Circuit::scope(name, || {
                            let _candidate = a.mod_wrapped(&b);
                            // assert_count_fails!(ModWrapped(Integer<I>, Integer<I>) => Integer<I>, &(mode_a, mode_b));
                            assert!(!Circuit::is_satisfied_in_scope(), "(!is_satisfied_in_scope)");
                        }),
                    }
                } else {
                    let expected = first.wrapping_mod(&second);
                    Circuit::scope(name, || {
                        let candidate = a.mod_wrapped(&b);
                        assert_eq!(expected, *candidate.eject_value());
                        assert_eq!(console::Integer::new(expected), candidate.eject_value());
                        // assert_count!(ModWrapped(Integer<I>, Integer<I>) => Integer<I>, &(mode_a, mode_b));
                        // assert_output_mode!(ModWrapped(Integer<I>, Integer<I>) => Integer<I>, &(mode_a, mode_b), candidate);
                        assert!(Circuit::is_satisfied_in_scope(), "(is_satisfied_in_scope)");
                    })
                }
            }
