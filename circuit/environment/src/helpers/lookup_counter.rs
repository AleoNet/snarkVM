...
pub(crate) struct LookupCounter<F: PrimeField> {
    scope: Scope,
    ...
    private: u64,
    gates: u64,
    parents: Vec<(Scope, Vec<LookupConstraint<F>>, u64, u64, u64, u64)>,
}

impl<F: PrimeField> LookupCounter<F> {
    ...
    pub(crate) fn pop<S: Into<String>>(&mut self, name: S) -> Result<(), String> {
        ...
        match current_scope == name.into() {
            true => {
                if let Some((scope, lookup_constraints, constants, public, private, gates)) = self.parents.pop() {
                    self.scope = scope;
    ...
    pub(crate) fn add_constraint(&mut self, constraint: LookupConstraint<F>) {
