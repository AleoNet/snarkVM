...
#[macro_export]
macro_rules! assert_scope_with_lookup {
    () => {{
        $crate::print_scope!();

        assert!(Circuit::is_satisfied_in_scope(), "(is_satisfied_in_scope)");
    }};
    ($num_constants:expr, $num_public:expr, $num_private:expr, $num_constraints:expr, $num_lookup_constraints:expr) => {{
        $crate::print_scope!();
        ...
        assert_eq!($num_private, Circuit::num_private_in_scope(), "(num_private)");
        assert_eq!($num_constraints, Circuit::num_constraints_in_scope(), "(num_constraints)");
        assert_eq!($num_lookup_constraints, Circuit::num_lookup_constraints_in_scope(), "(num_lookup_constraints)");
        assert!(Circuit::is_satisfied_in_scope(), "(is_satisfied_in_scope)");
    }};
    (<=$num_constants:expr, $num_public:expr, $num_private:expr, $num_constraints:expr, $num_lookup_constraints:expr) => {{
        $crate::print_scope!();
    ...
    (<=$num_constants:expr, <=$num_public:expr, <=$num_private:expr, <=$num_constraints:expr, <=$num_lookup_constraints:expr) => {{
        ...
        assert!(Circuit::num_private_in_scope() <= $num_private, "(num_private)");
        assert!(Circuit::num_constraints_in_scope() <= $num_constraints, "(num_constraints)");
        assert!(Circuit::num_lookup_constraints_in_scope() <= $num_lookup_constraints, "(num_lookup_constraints)");
        assert!(Circuit::is_satisfied_in_scope(), "(is_satisfied_in_scope)");
    }};
    ($case:expr, $num_constants:expr, $num_public:expr, $num_private:expr, $num_constraints:expr, $num_lookup_constraints:expr) => {{
        $crate::print_scope!();
        ...
        assert_eq!(
            ...
            Circuit::num_lookup_constraints_in_scope(),
            "{} (num_lookup_constraints)",
            $case
        );
        assert!(Circuit::is_satisfied_in_scope(), "{} (is_satisfied_in_scope)", $case);
