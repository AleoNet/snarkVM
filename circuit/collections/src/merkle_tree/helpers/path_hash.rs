...
pub trait PathHash<A: Aleo> {
    ...
    fn hash(&self, left: &Field<A>, right: &Field<A>) -> Field<A>;

    /// Returns the empty hash.
    fn hash_empty(&self) -> Field<A> {
        self.hash(&Field::zero(), &Field::zero())
    }
}
...
mod tests {
        ...
        ($hash:ident, $form:ident, $mode:ident, ($num_constants:expr, $num_public:expr, $num_private:expr, $num_constraints:expr)) => {{
            ...
            for i in 0..ITERATIONS {
                ...
                let expected: <Circuit as Environment>::BaseField =
                    console::merkle_tree::PathHash::<<Circuit as Aleo>::Network>::hash(&native, &left, &right)
                ...
                Circuit::scope(format!("PathHash {i}"), || {
                    ...
                    let candidate = PathHash::hash(&circuit, &left, &right);
    ...
    #[test]
    fn test_hash_bhp512_constant() -> Result<()> {
        check_hash!(BHP512, Affine, Constant, (1611, 0, 0, 0))
    }

    #[test]
    fn test_hash_bhp512_public() -> Result<()> {
        check_hash!(BHP512, Affine, Public, (421, 0, 1385, 1387))
    }

    #[test]
    fn test_hash_bhp512_private() -> Result<()> {
        check_hash!(BHP512, Affine, Private, (421, 0, 1385, 1387))
    }

    #[test]
    fn test_hash_poseidon2_constant() -> Result<()> {
        check_hash!(Poseidon2, BaseField, Constant, (1, 0, 0, 0))
    }

    #[test]
    fn test_hash_poseidon2_public() -> Result<()> {
        check_hash!(Poseidon2, BaseField, Public, (1, 0, 540, 540))
    }

    #[test]
    fn test_hash_poseidon2_private() -> Result<()> {
        check_hash!(Poseidon2, BaseField, Private, (1, 0, 540, 540))
    }
}
