...
use crate::{BatchCertificate, Transmission, TransmissionID};
use console::{
...
impl<N: Network> Batch<N> {
    ...
    pub const fn transmissions(&self) -> &HashMap<TransmissionID<N>, Transmission<N>> {
        &self.transmissions
    }
...
mod test_helpers {
    ...
    pub(crate) fn sample_batches(rng: &mut TestRng) -> Vec<Batch<CurrentNetwork>> {
        ...
        for _ in 0..10 {
            ...
            let transmissions = transmission_ids.into_iter().zip(transmissions.into_iter()).collect::<HashMap<_, _>>();
