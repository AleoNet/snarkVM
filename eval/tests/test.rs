...
use snarkvm_eval::{edwards_bls12::EdwardsGroupType, ConstrainedValue, Evaluator, Input, SetupEvaluator};
use snarkvm_ir::{
    Field,
    Group,
    GroupCoordinate,
...
fn mock_input(header: &Header, index: u32) -> Input {
...
fn setup_test_item(raw: &[u8]) -> Result<(TestConstraintSystem<Fr>, ConstrainedValue<Fr, EdwardsGroupType>)> {
