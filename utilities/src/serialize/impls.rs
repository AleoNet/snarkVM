...
impl<T: Valid> Valid for Option<T> {
    ...
    where
        Self: 'a,
    {
        T::batch_check(batch.map(Option::as_ref).filter(Option::is_some).map(Option::unwrap))
    }
}
...
// impl<'b, T: Valid> Valid for Cow<'b, T>
// where
//     T: ToOwned,
//     <T as ToOwned>::Owned: CanonicalDeserialize,
// {
//     #[inline]
//     fn check(&self) -> Result<Self, SerializationError> {
//         self.as_ref().check()
//     }
...
//         Self: 'a
//     {
//         <<T as ToOwned>::Owned>::batch_check(batch.iter().map(|v| v.as_ref()))
//     }
// }
...
impl<K: Valid, V: Valid> Valid for BTreeMap<K, V> {
        ...
        Self: 'a,
    {
        ...
        K::batch_check(keys.into_iter().flat_map(|k| k))?;
        V::batch_check(values.into_iter().flat_map(|v| v))
    }
}
