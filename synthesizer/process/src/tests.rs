...
mod staking_tests {
    ...
    fn test_bond_validator_to_other_validator_fails() {
        ...
        // Ensure that bonding to another validator fails.
        assert!(
            bond_public(&process, &finalize_store, &validator_private_key_1, &validator_address_2, amount, rng)
                .is_err()
        );
    }
    ...
    fn teat_bond_validator_multiple_bonds() {
        ...
        bond_public(&process, &finalize_store, delegator_private_key, validator_address, delegator_amount_1, rng)
            .unwrap();

        // Bond additional stake for the delegator.
        let delegator_amount_2 = 1_000_000u64;
        bond_public(&process, &finalize_store, delegator_private_key, validator_address, delegator_amount_2, rng)
            .unwrap();
    ...
    fn test_bond_delegator_to_multiple_validators_fails() {
        ...
        let delegator_amount = 1_000_000u64;
        bond_public(&process, &finalize_store, delegator_private_key, &validator_address_1, delegator_amount, rng)
            .unwrap();
