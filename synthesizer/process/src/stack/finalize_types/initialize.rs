...
impl<N: Network> FinalizeTypes<N> {
    ...
    pub(super) fn initialize_finalize_types(
        ...
        for command in finalize.commands() {
            ...
            // If the command is an `await`, check that the register is a future.
            if let Command::Await(await_) = command {
                // Note: `check_command` ensures that the register is a future. This is an additional check.
                if let FinalizeType::Plaintext(_) = finalize_types.get_type(stack, await_.register())? {
                    bail!("Expected a future in '{await_}'")
                }
            }
        }
