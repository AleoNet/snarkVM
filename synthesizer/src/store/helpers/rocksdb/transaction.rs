...
impl<N: Network> DeploymentStorage<N> for DeploymentDB<N> {
    ...
    fn open_testing(path: Option<std::path::PathBuf>, transition_store: TransitionStore<N, Self::TransitionStorage>) -> Result<Self> {
        Ok(Self {
            ...
            program_map: rocksdb::RocksDB::open_map_testing(path.clone(),  MapID::Deployment(DeploymentMap::Program))?,
            verifying_key_map: rocksdb::RocksDB::open_map_testing(path.clone(),  MapID::Deployment(DeploymentMap::VerifyingKey))?,
            certificate_map: rocksdb::RocksDB::open_map_testing(path.clone(),  MapID::Deployment(DeploymentMap::Certificate))?,
            fee_map: rocksdb::RocksDB::open_map_testing(path.clone(),  MapID::Deployment(DeploymentMap::Fee))?,
            reverse_fee_map: rocksdb::RocksDB::open_map_testing(path,  MapID::Deployment(DeploymentMap::ReverseFee))?,
            transition_store,
        })
    }
