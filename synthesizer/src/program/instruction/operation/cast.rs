...
mod tests {
    ...
    fn check_cast<const VARIANT: u8>(
        ...
        // Initialize the stack.
        let (stack, operands, destination) = sample_stack(opcode, type_, *mode, output_type, cache).unwrap();
        ...
        ($input_type:ident, $output_type:ident) => {
            paste::paste! {
                ...
                fn [<test _ cast _ from _ $input_type:lower _ to _ $output_type:lower _ is _ consistent>]() {
                    ...
                    let cast_operation = |operands, destination, output_type| Cast::<CurrentNetwork> { operands, destination, cast_type: CastType::RegisterType(RegisterType::Plaintext(PlaintextType::Literal(output_type))) };
                    let cast_lossy_operation = |operands, destination, output_type| CastLossy::<CurrentNetwork> { operands, destination, cast_type: CastType::RegisterType(RegisterType::Plaintext(PlaintextType::Literal(output_type))) };
                    ...
                    for _ in 0..ITERATIONS {
                        ...
                        for mode in modes.iter() {
                            check_cast(cast_operation, cast_opcode, &literal, mode, LiteralType::$output_type, &mut cache);
                            check_cast(cast_lossy_operation, cast_lossy_opcode, &literal, mode, LiteralType::$output_type, &mut cache);
                        }
                    }
                }
            }
