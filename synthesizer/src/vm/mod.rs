...
finalize do:
    ...
    fn test_deployment_exceeding_max_transaction_spend() {
        ...
        for i in 0..<CurrentNetwork as Network>::MAX_COMMANDS.ilog2() {
            ...
            let stack = Stack::<CurrentNetwork>::new(&vm.process().read(), &program).unwrap();
        ...
        let deployment = vm.deploy(&private_key, &allowed_program, None, 0, None, rng).unwrap();
        ...
        // Check that the block has one accepted transaction.
        ...
        assert!(vm.contains_program(allowed_program.id()));

        // Deploy the exceeding program.
        let deployment = vm.deploy(&private_key, &exceeding_program, None, 0, None, rng).unwrap();
