...
use crate::{
    prelude::{Stack, StackProgram, StackProgramTypes},
    VM,
};
use console::{
    prelude::*,
...
use synthesizer_program::{Command, Finalize, Instruction, Operand};
...
pub fn execution_cost<N: Network, C: ConsensusStorage<N>>(
    ...
    for transition in execution.transitions() {
        ...
        // Retrieve the finalize cost for the command.
        let cost = match program.get_function(function_name)?.finalize() {
            Some((_, finalize)) => {
                finalize_cost_in_microcredits(vm.process().read().get_stack(program.id())?, finalize)?
            }
            None => continue,
        };
