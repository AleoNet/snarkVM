...
pub fn cost_in_microcredits<N: Network>(finalize: &Finalize<N>) -> Result<u64> {
    ...
    let cost = |command: &Command<N>| match command {
        ...
        Command::Instruction(Instruction::CallClosure(_)) => bail!("`call.closure` is not supported in finalize."),
        Command::Instruction(Instruction::CallFinalize(_)) => Ok(2_000),
        Command::Instruction(Instruction::CallFunction(_)) => bail!("`call.function` is not supported in finalize."),
