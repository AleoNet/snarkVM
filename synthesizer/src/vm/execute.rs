...
mod tests {
    ...
    fn test_transfer_private_transaction_size() {
        ...
        assert_eq!(3524, transaction_size_in_bytes, "Update me if serialization has changed");
        ...
        if let Transaction::Execute(_, execution, _) = &transaction {
            ...
            assert_eq!(2211, execution_size_in_bytes, "Update me if serialization has changed");
        }
    }
    ...
    fn test_transfer_public_transaction_size() {
        ...
        // Assert the size of the transaction.
        let transaction_size_in_bytes = transaction.to_bytes_le().unwrap().len();
        assert_eq!(2633, transaction_size_in_bytes, "Update me if serialization has changed");

        // Assert the size of the execution.
        assert!(matches!(transaction, Transaction::Execute(_, _, _)));
        if let Transaction::Execute(_, execution, _) = &transaction {
            let execution_size_in_bytes = execution.to_bytes_le().unwrap().len();
            assert_eq!(1320, execution_size_in_bytes, "Update me if serialization has changed");
        }
    }
    ...
    fn test_join_transaction_size() {
        ...
        // Assert the size of the transaction.
        let transaction_size_in_bytes = transaction.to_bytes_le().unwrap().len();
        assert_eq!(3369, transaction_size_in_bytes, "Update me if serialization has changed");
    ...
    fn test_split_transaction_size() {
        ...
        // Assert the size of the transaction.
        let transaction_size_in_bytes = transaction.to_bytes_le().unwrap().len();
        assert_eq!(2135, transaction_size_in_bytes, "Update me if serialization has changed");

        // Assert the size of the execution.
        assert!(matches!(transaction, Transaction::Execute(_, _, _)));
        if let Transaction::Execute(_, execution, _) = &transaction {
            let execution_size_in_bytes = execution.to_bytes_le().unwrap().len();
            assert_eq!(2100, execution_size_in_bytes, "Update me if serialization has changed");
        }
    }
    ...
    fn test_fee_private_transition_size() {
        ...
        assert_eq!(1967, fee_size_in_bytes, "Update me if serialization has changed");
    }
    ...
    fn test_fee_public_transition_size() {
        ...
        // Assert the size of the transition.
        let fee_size_in_bytes = fee.to_bytes_le().unwrap().len();
        assert_eq!(1278, fee_size_in_bytes, "Update me if serialization has changed");
    }
}
