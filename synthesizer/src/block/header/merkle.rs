...
impl<N: Network> Header<N> {
    ...
    pub fn to_leaf(&self, id: &Field<N>) -> Result<HeaderLeaf<N>> {
        ...
        // If the ID is the storage root, return the 2nd leaf.
        else if id == &self.storage_root {
            Ok(HeaderLeaf::<N>::new(2, self.storage_root))
        }
    ...
    pub fn to_tree(&self) -> Result<HeaderTree<N>> {
        ...
        leaves.push(HeaderLeaf::<N>::new(2, self.storage_root).to_bits_le());
