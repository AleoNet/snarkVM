...
fn bench_mint_public(c: &mut Criterion) {
    ...
    bench_speculate_commit_and_finalize(
        ...
        &[],
        &[],
        &[(ProgramID::from_str("test.aleo").unwrap(), Identifier::from_str("mint_public").unwrap(), vec![
            address.clone(),
            amount.clone(),
        ])],
        NUM_EXECUTIONS,
...
fn bench_transfer_public(c: &mut Criterion) {
    ...
    bench_speculate_commit_and_finalize(
        ...
        &[],
        &[(ProgramID::from_str("test.aleo").unwrap(), Identifier::from_str("transfer_public").unwrap(), vec![
            sender.clone(),
            recipient.clone(),
            amount.clone(),
        ])],
