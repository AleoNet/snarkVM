...
fn bench_one_operation(c: &mut Criterion) {
    ...
    let mut workload = Workload::new("commit_one_operation".to_string(), vec![]).unwrap();
    ...
    bench_commit::<ConsensusMemory<Testnet3>>(c, "memory", workload);
    #[cfg(any(feature = "rocks"))]
    bench_commit::<snarkvm_synthesizer::helpers::rocksdb::ConsensusDB<Testnet3>>(c, "db", workload);
}

fn bench_multiple_operations(c: &mut Criterion) {
    // Initialize the workloads.
    let mut workload = Workload::new("commit_multiple_operations".to_string(), vec![]).unwrap();
...
fn bench_multiple_operations_with_multiple_programs(c: &mut Criterion) {
    ...
    let mut workload = Workload::new("commit_multiple_operations_with_multiple_programs".to_string(), vec![]).unwrap();
