...
fn check_number_of_operands<N: Network>(opcode: Opcode, num_operands: usize, cast_type: CastType<N>) -> Result<()> {
    match cast_type {
        CastType::GroupXCoordinate => {
            ensure!(
                num_operands == 1,
                "Instruction '{opcode}' - Casting to a group x-coordinate requires exactly 1 operand, found {num_operands} operands"
            );
        }
        CastType::GroupYCoordinate => {
            ensure!(
                num_operands == 1,
                "Instruction '{opcode}' - Casting to a group y-coordinate requires exactly 1 operand, found {num_operands} operands"
            );
        }
        CastType::RegisterType(RegisterType::Plaintext(PlaintextType::Literal(literal_type))) => {
            ensure!(
                num_operands == 1,
                "Instruction '{opcode}' - Casting to a literal '{literal_type}' requires exactly 1 operand"
            );
        }
        CastType::RegisterType(RegisterType::Plaintext(PlaintextType::Struct(struct_name))) => {
            ...
            if num_operands < N::MIN_STRUCT_ENTRIES {
                bail!(
                    "Instruction '{opcode}' - Casting to struct '{struct_name}' requires at least {} operand(s), found {num_operands} operands",
        ...
        CastType::RegisterType(RegisterType::Record(record_name)) => {
            // Ensure the operands length is at least the minimum.
            if num_operands < N::MIN_RECORD_ENTRIES {
                bail!(
                    "Instruction '{opcode}' - Casting to record '{record_name}' requires at least {} operand(s), found {num_operands} operands",
                    N::MIN_RECORD_ENTRIES
                )
            }
        }
        CastType::RegisterType(RegisterType::ExternalRecord(_locator)) => {
            bail!("Instruction '{opcode}' - Illegal operation: Cannot cast to an external record.")
        }
    }
    Ok(())
}
...
impl<N: Network, const VARIANT: u8> CastInstruction<N, VARIANT> {
    ...
    pub fn new(operands: Vec<Operand<N>>, destination: Register<N>, cast_type: CastType<N>) -> Result<Self> {
        ...
        check_number_of_operands(Self::opcode(), operands.len(), cast_type)?;
    ...
    #[inline]
    pub fn destination_type(&self) -> &RegisterType<N> {
