...
impl<N: Network> Delete<N> {
    /// Initializes a new `delete` instruction.
    #[inline]
...
impl<N: Network> Delete<N> {
    ...
    pub fn evaluate(
        ...
        bail!("`delete` is currently only supported in `finalize`")
    }
...
impl<N: Network> Parser for Delete<N> {
...
impl<N: Network> FromStr for Delete<N> {
    type Err = Error;
...
impl<N: Network> Debug for Delete<N> {
...
impl<N: Network> Display for Delete<N> {
...
impl<N: Network> FromBytes for Delete<N> {
...
impl<N: Network> ToBytes for Delete<N> {
...
mod tests {
    ...
    #[test]
    fn test_parse() {
        let (string, delete) = Delete::<CurrentNetwork>::parse("delete r0 r1 into r2").unwrap();
        assert!(string.is_empty(), "Parser did not consume all of the string: '{string}'");
        assert_eq!(delete.operands.len(), 2);
        assert_eq!(delete.operands[0], Operand::Register(Register::Locator(0)), "The first operand is incorrect");
        assert_eq!(delete.operands[1], Operand::Register(Register::Locator(1)), "The second operand is incorrect");
        assert_eq!(delete.destination, Register::Locator(2), "The destination register is incorrect");
    }
}
