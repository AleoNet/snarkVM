...
impl<N: Network> Parser for Import<N> {
    ...
    /// The import statement is of the form `import {name}.{domain};`.
    #[inline]
    fn parse(string: &str) -> ParserResult<Self> {
        ...
        // Parse the domain from the string.
        let (string, domain) = Identifier::parse(string)?;
        // Parse the whitespace from the string.
        let (string, _) = Sanitizer::parse_whitespaces(string)?;
        // Parse the semicolon from the string.
        let (string, _) = tag(";")(string)?;
        // Return the import statement.
        Ok((string, Self { name, domain }))
    }
}
...
impl<N: Network> Display for Import<N> {
    ...
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        write!(f, "{type_} {name}.{domain};", type_ = Self::type_name(), name = self.name, domain = self.domain)
    }
}
...
mod tests {
    ...
    fn test_import_parse() -> Result<()> {
        ...
        assert_eq!(import.domain(), &Identifier::<CurrentNetwork>::from_str("aleo")?);
