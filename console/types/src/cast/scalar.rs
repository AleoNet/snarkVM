...
impl<E: Environment, I: IntegerType> Cast<Integer<E, I>> for Scalar<E> {
    fn cast(&self) -> Result<Integer<E, I>> {
        ...
        // Check that the upper bits are zero.
        for bit in bits_le.iter().skip(usize::try_from(I::BITS)?) {
            if *bit {
                bail!(format!("Failed to convert scalar to integer '{}': scalar is out of bounds", I::type_name()))
            }
        }
        // If the target integer type is signed, also check that the `I::BITS`-th bit is zero.
        if I::is_signed() && bits_le[usize::try_from(I::BITS)?] {
            bail!(format!("Failed to convert scalar to integer '{}': scalar is out of bounds", I::type_name()))
        }
