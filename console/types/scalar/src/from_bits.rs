...
mod tests {
    ...
    fn check_from_bits_le() -> Result<()> {
        let expected_size_in_bits = <CurrentNetwork as Network>::Scalar::size_in_bits();

        for i in 0..ITERATIONS {
            // Sample a random element.
            let expected = Uniform::rand(&mut test_rng());
            let given_bits = Scalar::<CurrentNetwork>::new(expected).to_bits_le();
    ...
    fn check_from_bits_be() -> Result<()> {
        ...
        for i in 0..ITERATIONS {
            // Sample a random element.
            let expected = Uniform::rand(&mut test_rng());
            let given_bits = Scalar::<CurrentNetwork>::new(expected).to_bits_be();
            assert_eq!(expected_size_in_bits, given_bits.len());
