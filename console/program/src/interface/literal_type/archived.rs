...
impl<A: Aleo> From<Literal<P::Aleo>> for LiteralType<A> {
    #[inline]
    fn from(literal: Literal<P::Aleo>) -> Self {
        Self::from(&literal)
    }
}
...
impl<A: Aleo> Parser for LiteralType<A> {
    type Environment = A;
    ...
    fn parse(string: &str) -> ParserResult<Self> {
        ...
        let result = alt((
            map(pair(pair(tag(Address::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
                Self::Address(mode)
            }),
            map(pair(pair(tag(Boolean::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
                Self::Boolean(mode)
            }),
            map(pair(pair(tag(Field::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
                Self::Field(mode)
            }),
            map(pair(pair(tag(Group::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
                Self::Group(mode)
            }),
            map(pair(pair(tag(I8::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| Self::I8(mode)),
            map(pair(pair(tag(I16::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
                Self::I16(mode)
            }),
            map(pair(pair(tag(I32::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
                Self::I32(mode)
            }),
            map(pair(pair(tag(I64::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
                Self::I64(mode)
            }),
            map(pair(pair(tag(I128::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
                Self::I128(mode)
            }),
            map(pair(pair(tag(U8::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| Self::U8(mode)),
            map(pair(pair(tag(U16::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
                Self::U16(mode)
            }),
            map(pair(pair(tag(U32::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
                Self::U32(mode)
            }),
            map(pair(pair(tag(U64::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
                Self::U64(mode)
            }),
            map(pair(pair(tag(U128::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
                Self::U128(mode)
            }),
            map(pair(pair(tag(Scalar::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
                Self::Scalar(mode)
            }),
            map(pair(pair(tag(StringType::<A>::type_name()), tag(".")), Mode::parse), |(_, mode)| {
...
impl<A: Aleo> Debug for LiteralType<A> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}.{}", self.type_name(), self.mode())
    }
}

impl<A: Aleo> Display for LiteralType<A> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}.{}", self.type_name(), self.mode())
    }
}

impl<A: Aleo> FromBytes for LiteralType<A> {
...
impl<A: Aleo> ToBytes for LiteralType<A> {
