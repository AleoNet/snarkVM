...
impl<N: Network> ToFields for Request<N> {
    type Field = Field<N>;
    ...
    fn to_fields(&self) -> Result<Vec<Self::Field>> {
        ...
        fields.extend(
            self.inputs.iter().map(|input| input.to_fields()).collect::<Result<Vec<_>>>()?.into_iter().flatten(),
        );

        // Ensure the number of field elements does not exceed the maximum allowed size.
        match fields.len() <= N::MAX_DATA_SIZE_IN_FIELDS as usize {
            true => Ok(fields),
            false => bail!("Request exceeds maximum allowed size"),
        }
    }
}
