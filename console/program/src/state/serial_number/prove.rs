...
impl<N: Network> SerialNumber<N> {
    ...
    pub fn prove<R: Rng + CryptoRng>(sk_vrf: &N::Scalar, state_digest: N::Field, rng: &mut R) -> Result<Self> {
        ...
        let generator_h = N::hash_to_group_psd2(&[N::serial_number_domain(), state_digest])?;
        ...
        // Compute `serial_number` as `Hash(state_digest || serial_number_nonce)`.
        let serial_number = N::hash_bhp512(&[state_digest, serial_number_nonce].to_bits_le())?;
