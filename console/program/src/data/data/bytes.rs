...
impl<N: Network> FromBytes for Data<N> {
    ...
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        ...
        match num_fields as u32 <= N::MAX_DATA_SIZE_IN_FIELDS {
            ...
            true => Ok(Data((0..num_fields).map(|_| Field::read_le(&mut reader)).collect::<Result<Vec<_>, _>>()?)),
            false => Err(error("Data is too large to encode in field elements.")),
        }
    }
}

impl<N: Network> ToBytes for Data<N> {
    ...
    fn write_le<W: Write>(&self, mut writer: W) -> IoResult<()> {
        ...
        if u32::try_from(self.0.len()).or_halt::<N>() > N::MAX_DATA_SIZE_IN_FIELDS || self.0.len() > u16::MAX as usize {
            return Err(error("Data is too large to encode in field elements."));
        }
        // Write the number of data field elements.
        u16::try_from(self.0.len()).or_halt::<N>().write_le(&mut writer)?;
        // Write the data field elements.
...
mod tests {
    ...
    fn test_bytes() -> Result<()> {
        ...
        for _ in 0..ITERATIONS {
            ...
            assert_eq!(expected, Data::read_le(&expected_bytes[..])?);
        }
        Ok(())
    }
}
