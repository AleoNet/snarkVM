...
impl<N: Network> Balance<N, Plaintext<N>> {
    ...
    pub fn to_value(&self) -> Value<N, Plaintext<N>> {
        match self {
            Self::Public(balance) => Value::Public(Plaintext::from(Literal::U64(*balance))),
    ...
    pub fn decrypt(&self, randomizer: &[Field<N>]) -> Result<Balance<N, Plaintext<N>>> {
        match self {
            ...
            Self::Private(ciphertext) => {
                ...
                ensure!(ciphertext.len() == 1, "Expected 1 randomizer, found {}", ciphertext.len());
                // Decrypt the balance.
        ...
        match self {
            ...
            Self::Private(ciphertext) => {
                ...
                match ciphertext.len() == 1 {
                    ...
                    false => N::halt(format!("Internal error: ciphertext to_bits_le corrupted in record balance")),
                }
            }
        }
        bits_le
        ...
        match self {
            ...
            Self::Private(ciphertext) => {
                ...
                match ciphertext.len() == 1 {
                    ...
                    false => N::halt(format!("Internal error: ciphertext to_bits_be corrupted in record balance")),
                }
            }
        }
        bits_be
