        ...
        match self {
            ...
            Self::Interface(interface, bits_le) => bits_le
                .get_or_init(|| {
                    ...
                    bits_le.extend(u8::try_from(interface.len()).unwrap().to_bits_le());
                    for (identifier, value) in interface {
                        ...
                        bits_le.extend(u16::try_from(value_bits.len()).unwrap().to_bits_le());
        ...
        match self {
            ...
            Self::Interface(interface, bits_be) => bits_be
                .get_or_init(|| {
                    ...
                    bits_be.extend(u8::try_from(interface.len()).unwrap().to_bits_be());
                    for (identifier, value) in interface {
                        let value_bits = value.to_bits_be();
                        bits_be.extend(identifier.size_in_bits().to_bits_be());
                        bits_be.extend(identifier.to_bits_be());
                        bits_be.extend(u8::try_from(value_bits.len()).unwrap().to_bits_be());
