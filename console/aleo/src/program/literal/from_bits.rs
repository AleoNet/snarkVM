...
impl<E: Environment> FromBits for Literal<E> {
    type Boolean = Boolean<E>;

    /// Initializes a new literal from a list of little-endian bits *without* trailing zeros.
    fn from_bits_le(bits_le: &[Boolean<E>]) -> Self {
        let (variant, literal) = bits_le.split_at(8);
        let variant = U8::from_bits_le(variant);
        match variant.eject_value() {
            ...
            16.. => E::halt(format!("Failed to initialize literal variant {} from bits (LE)", variant.eject_value())),
        }
    }

    /// Initializes a new literal from a list of big-endian bits *without* leading zeros.
    fn from_bits_be(bits_be: &[Boolean<E>]) -> Self {
        let (variant, literal) = bits_be.split_at(8);
        let variant = U8::from_bits_be(variant);
        match variant.eject_value() {
            ...
            16.. => E::halt(format!("Failed to initialize literal variant {} from bits (BE))", variant.eject_value())),
        }
    }
}
