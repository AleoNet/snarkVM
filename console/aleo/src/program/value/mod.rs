...
use crate::{Address, Network, Literal, ViewKey};
...
pub enum Value<N: Network, Private: ValueType> {
    /// A constant value.
...
impl<N: Network, Private: ValueType> Value<N, Private> {
...
impl<N: Network> ValueType for Plaintext<N> {
    /// Returns the recursive depth of this public value.
    /// Note: Once `generic_const_exprs` is stabilized, this can be replaced with `const DEPTH: u8`.
    fn depth(&self, counter: usize) -> usize {
        match self {
...
impl<N: Network> ValueType for Ciphertext<N> {
