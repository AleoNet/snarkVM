...
impl<N: Network> State<N> {
    ...
    pub fn encrypt(&self, randomizer: &N::Scalar) -> Result<Record<N>> {
        // Ensure the nonce corresponds to the given randomizer: `nonce == randomizer * G`.
            ...
            self.nonce == N::g_scalar_multiply(randomizer).to_affine(),
            "Attempted to encrypt using an invalid randomizer"
        );
        // Compute the record view key as `randomizer * address`.
        let record_view_key = (*self.owner * *randomizer).to_affine().to_x_coordinate();
