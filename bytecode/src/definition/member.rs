...
pub struct Member<P: Program> {
    ...
    // TODO: If definitions are not allowed to be nested, then this should be changed `LiteralType`.
    //  Using `Annotation` implies that a `Member` can be a definition type and thus we can have
    //  r0.<id1>.<id2>.
