...
use crate::Program;
use snarkvm_circuits::{algorithms::Pedersen128 as Pedersen128Gadget, CommitmentScheme, Literal, ToBits};

const SETUP_MESSAGE: &str = "PedersenCommitment128";

#[derive(Clone)]
pub struct Pedersen128<P: Program>(Pedersen128Gadget<P::Environment>);

impl<P: Program> Default for Pedersen128<P> {
    fn default() -> Self {
...
impl<P: Program> Pedersen128<P> {
    ...
    pub fn new(message: &str) -> Self {
        Self(Pedersen128Gadget::setup(message))
    }
        ...
        if let Literal::Scalar(r) = randomness {
            match value {
                ...
                Literal::String(a) => {
                    ...
                    if bits.len() > 128 {
                        P::halt("Invalid input size for Pedersen128 commitment")
                    } else {
                        Literal::Group(self.0.commit(&bits, &r.to_bits_le()))
                    }
                }
                _ => P::halt("Invalid input size for Pedersen128 commitment"),
            }
        } else {
            P::halt("Invalid randomness type for Pedersen128 commitment")
        }
    }
}
