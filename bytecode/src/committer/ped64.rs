...
use crate::Program;
use snarkvm_circuits::{algorithms::Pedersen64 as Pedersen64Gadget, CommitmentScheme, Literal, ToBits};

const SETUP_MESSAGE: &str = "PedersenCommitment64";
        ...
        if let Literal::Scalar(r) = randomness {
            match value {
                ...
                Literal::String(a) => {
                    ...
                    if bits.len() > 64 {
                        P::halt("Invalid input size for Pedersen64 commitment")
                    } else {
                        Literal::Group(self.0.commit(&bits, &r.to_bits_le()))
                    }
                }
                _ => P::halt("Invalid input size for Pedersen64 commitment"),
            }
        } else {
            P::halt("Invalid randomness type for Pedersen64 commitment")
        }
    }
}
