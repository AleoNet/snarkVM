...
mod tests {
    use super::*;
    use crate::{binary_instruction_test, test_instruction_halts, test_modes, Identifier, Process, Value};

    #[test]
    fn test_parse() {
        let (_, instruction) = Instruction::<Process>::parse("gt r0 r1 into r2;").unwrap();
        assert!(matches!(instruction, Instruction::GreaterThan(_)));
    }
