...
impl<P: Program> FromBytes for Instruction<P> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        ...
        match code {
            ...
            29 => Ok(Self::Sub(Sub::read_le(&mut reader)?)),
            30 => Ok(Self::SubWrapped(SubWrapped::read_le(&mut reader)?)),
            31 => Ok(Self::Ternary(Ternary::read_le(&mut reader)?)),
            32 => Ok(Self::Xor(Xor::read_le(&mut reader)?)),
