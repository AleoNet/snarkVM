...
use snarkvm_circuit::{Literal, Parser, ParserResult, ShrChecked};
...
mod tests {
    use super::*;
    use crate::{test_instruction_halts, test_modes, Identifier, Process, Register, Value};
    ...
    #[test]
    fn test_parse() {
        let (_, instruction) = Instruction::<P>::parse("shr r0 r1 into r2;").unwrap();
        assert!(matches!(instruction, Instruction::Shr(_)));
    }
