        ...
        match self {
            ...
            Self::REST(url) => match N::ID {
                ...
                console::network::TestnetV1::ID => {
                    Ok(Self::get_request(&format!("{url}/testnet_v1/latest/stateRoot"))?.into_json()?)
                }
    ...
    async fn current_state_root_async(&self) -> Result<N::StateRoot> {
        match self {
            ...
            Self::REST(url) => match N::ID {
                ...
                console::network::TestnetV1::ID => {
                    Ok(Self::get_request_async(&format!("{url}/testnet_v1/latest/stateRoot")).await?.json().await?)
                }
                _ => bail!("Unsupported network ID in inclusion query"),
            },
        }
    ...
    fn get_state_path_for_commitment(&self, commitment: &Field<N>) -> Result<StatePath<N>> {
        match self {
            ...
            Self::REST(url) => match N::ID {
                ...
                console::network::TestnetV1::ID => {
                    Ok(Self::get_request(&format!("{url}/testnet_v1/statePath/{commitment}"))?.into_json()?)
                }
                _ => bail!("Unsupported network ID in inclusion query"),
            },
        }
    ...
    async fn get_state_path_for_commitment_async(&self, commitment: &Field<N>) -> Result<StatePath<N>> {
        match self {
            ...
            Self::REST(url) => match N::ID {
                ...
                console::network::TestnetV1::ID => {
                    Ok(Self::get_request_async(&format!("{url}/testnet_v1/statePath/{commitment}"))
                        .await?
                        .json()
                        .await?)
                }
    ...
    pub fn get_program(&self, program_id: &ProgramID<N>) -> Result<Program<N>> {
        match self {
            ...
            Self::REST(url) => match N::ID {
                ...
                console::network::TestnetV1::ID => {
                    Ok(Self::get_request(&format!("{url}/testnet_v1/program/{program_id}"))?.into_json()?)
                }
                _ => bail!("Unsupported network ID in inclusion query"),
            },
        }
    ...
    pub async fn get_program_async(&self, program_id: &ProgramID<N>) -> Result<Program<N>> {
        match self {
            ...
            Self::REST(url) => match N::ID {
                ...
                console::network::TestnetV1::ID => {
                    Ok(Self::get_request_async(&format!("{url}/testnet_v1/program/{program_id}")).await?.json().await?)
                }
                _ => bail!("Unsupported network ID in inclusion query"),
            },
        }
