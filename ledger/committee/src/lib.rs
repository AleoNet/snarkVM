...
pub mod test_helpers {
    ...
    pub fn sample_committee_for_round_and_size(
        ...
        for _ in 0..num_members {
            let is_open = rng.gen();
            members.insert(Address::<CurrentNetwork>::new(rng.gen()), (2 * MIN_VALIDATOR_STAKE, is_open, 0u8));
        }
    ...
    pub fn sample_committee_for_round_and_members(
        ...
        for member in members {
            let is_open = rng.gen();
            committee_members.insert(member, (2 * MIN_VALIDATOR_STAKE, is_open, 0u8));
        }
        // Return the committee.
    ...
    pub fn sample_committee_custom(num_members: u16, rng: &mut TestRng) -> Committee<CurrentNetwork> {
        ...
        members.insert(Address::<CurrentNetwork>::new(rng.gen()), (MIN_VALIDATOR_STAKE, false, 0u8));
        while members.len() < num_members as usize - 1 {
            loop {
                ...
                if stake >= MIN_VALIDATOR_STAKE as f64 && stake <= MAX_STAKE as f64 {
                    let is_open = rng.gen();
                    members.insert(Address::<CurrentNetwork>::new(rng.gen()), (stake as u64, is_open, 0u8));
                    break;
                }
            }
        }
        members.insert(Address::<CurrentNetwork>::new(rng.gen()), (MAX_STAKE, false, 0u8));
        // Return the committee.
