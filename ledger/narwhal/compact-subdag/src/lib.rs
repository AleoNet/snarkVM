...
use narwhal_compact_batch_certificate::CompactBatchCertificate;
...
/// Returns `true` if the DFS traversal using the given compact subdag structure matches the commit.
fn sanity_check_subdag_with_dfs<N: Network>(subdag: &BTreeMap<u64, IndexSet<CompactBatchCertificate<N>>>) -> bool {
...
pub struct CompactSubdag<N: Network> {
    /// The subdag of round certificates.
    subdag: BTreeMap<u64, IndexSet<CompactBatchCertificate<N>>>,
}

impl<N: Network> CompactSubdag<N> {
    /// Initializes a new subdag.
    pub fn from(subdag: BTreeMap<u64, IndexSet<CompactBatchCertificate<N>>>) -> Result<Self> {
    ...
    /// Initializes a new compact subdag from a subdag.
    pub fn from_subdag(subdag: &Subdag<N>) -> Result<Self> {
        // Construct the compact subdag.
        let compact_subdag = subdag
            .iter()
            .map(|(round, certificates)| {
                let compact_certificates = certificates
                    .iter()
                    .map(CompactBatchCertificate::from_batch_certificate)
                    .collect::<Result<IndexSet<_>>>()?;
                Ok((*round, compact_certificates))
            })
            .collect::<Result<BTreeMap<_, _>>>()?;

        // Initialize a compact subdag from the map.
        Self::from(compact_subdag)
    }
}
...
impl<N: Network> CompactSubdag<N> {
    ...
    pub fn leader_certificate(&self) -> &CompactBatchCertificate<N> {
    ...
    pub fn transmission_ids_map(&self) -> impl Iterator<Item = &(TransmissionType, u32)> {
        self.values().flatten().flat_map(CompactBatchCertificate::transmission_ids_map)
    }
...
impl<N: Network> Deref for CompactSubdag<N> {
    type Target = BTreeMap<u64, IndexSet<CompactBatchCertificate<N>>>;
...
pub mod test_helpers {
    ...
    pub fn sample_subdag(rng: &mut TestRng) -> CompactSubdag<CurrentNetwork> {
        ...
        for _ in 0..AVAILABILITY_THRESHOLD {
            let certificate =
                narwhal_compact_batch_certificate::test_helpers::sample_compact_batch_certificate_for_round(
                    starting_round,
                    rng,
                );
        ...
        for _ in 0..QUORUM_THRESHOLD {
            let certificate =
                narwhal_compact_batch_certificate::test_helpers::sample_compact_batch_certificate_for_round_with_previous_certificate_ids(starting_round + 1, previous_certificate_ids.clone(), rng);
        ...
        let certificate =
            narwhal_compact_batch_certificate::test_helpers::sample_compact_batch_certificate_for_round_with_previous_certificate_ids(
                starting_round + 2,
...
pub mod tests {
    ...
    fn test_from_subdag() {
        ...
        // Sample subdags.
        let subdags = narwhal_subdag::test_helpers::sample_subdags(rng);

        for subdag in subdags {
            // Convert the subdag to a compact subdag.
            let compact_batch_header = CompactSubdag::from_subdag(&subdag).unwrap();

            // Check tat the compact subdag fields are correct.
            for ((expected_timestamp, expected_certificates), (timestamp, certificates)) in
                subdag.iter().zip_eq(compact_batch_header.iter())
            {
                assert_eq!(expected_timestamp, timestamp);
                for (expected_certificate, certificate) in expected_certificates.iter().zip_eq(certificates.iter()) {
                    assert_eq!(expected_certificate.certificate_id(), certificate.certificate_id());
                }
            }
        }
    }
