...
impl<N: Network> FromBytes for CompactSubdag<N> {
    ...
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        ...
        for _ in 0..num_rounds {
            ...
            for _ in 0..num_certificates {
                // Read the compact certificate.
                certificates.insert(CompactBatchCertificate::read_le(&mut reader)?);
            }
...
impl<N: Network> ToBytes for CompactSubdag<N> {
...
mod tests {
    ...
    fn test_bytes() {
        ...
        for expected in crate::test_helpers::sample_compact_subdags(rng) {
            ...
            assert!(CompactSubdag::<CurrentNetwork>::read_le(&expected_bytes[1..]).is_err());
        }
    }
}
