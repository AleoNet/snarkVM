...
use crate::{helpers::anchor_block_height, Ledger};
use console::{network::Network, prelude::Itertools};
...
pub struct PendingSolutions<N: Network> {
    /// The pool of pending solutions and their proof targets.
    pending_solutions: Arc<RwLock<HashMap<PuzzleCommitment<N>, (ProverSolution<N>, u64)>>>,
}

impl<N: Network> PendingSolutions<N> {
    /// Initializes a new pool for the pending solutions.
    pub fn new() -> Self {
        Self { pending_solutions: Default::default() }
    }
}

impl<N: Network> Default for PendingSolutions<N> {
    fn default() -> Self {
        Self::new()
    }
}
...
impl<N: Network> PendingSolutions<N> {
    /// Returns `true` if the given pending solution exists in the pool.
    pub fn contains_pending_solution(&self, puzzle_commitment: PuzzleCommitment<N>) -> bool {
        self.pending_solutions.read().contains_key(&puzzle_commitment)
    }

    /// Returns the number of pending solutions in the pool.
    pub fn num_pending_solutions(&self) -> usize {
        self.pending_solutions.read().len()
    }

    /// Returns the pending solutions in the pool.
    pub fn pending_solutions(&self) -> Vec<(ProverSolution<N>, u64)> {
        self.pending_solutions.read().values().cloned().collect()
    }

    /// Returns the candidate coinbase target of the valid pending solutions in the pool.
    ...
    /// Returns a candidate set of pending solutions for inclusion in a block.
    pub fn candidate_solutions<C: ConsensusStorage<N>>(
        ...
        let candidate_solutions: Vec<_> = self
            ...
            .filter(|(_, (solution, _))| {
                ...
                match ledger.contains_puzzle_commitment(&solution.commitment()) {
                    Ok(true) => false,
                    Ok(false) => true,
                    Err(error) => {
    ...
    /// Adds the given pending solution to the pool, returning `true` if the solution was added to the pool.
    pub fn add_pending_solution(&self, solution: ProverSolution<N>) -> Result<bool> {
        // Acquire the write lock on the pending solutions.
        let mut pending_solutions = self.pending_solutions.write();

        // Ensure the solution does not already exist in the pool.
        match pending_solutions.entry(solution.commitment()) {
            Entry::Vacant(entry) => {
                ...
                debug!("✉️  Added a prover solution with target '{proof_target}' to the pool");
                Ok(true)
            }
            Entry::Occupied(_) => {
                trace!("Prover solution '{}' already exists in pool", solution.commitment());
                Ok(false)
            }
        }
    }

    /// Clears the pool of pending transactions that are now invalid.
    pub fn clear_invalid_solutions<C: ConsensusStorage<N>>(&self, ledger: &Ledger<N, C>) {
        self.pending_solutions.write().retain(|puzzle_commitment, _solution| {
            // Ensure the prover solution is still valid.
            match ledger.contains_puzzle_commitment(puzzle_commitment) {
                Ok(true) | Err(_) => {
                    trace!("Removed prover solution '{puzzle_commitment}' from the pool");
                    false
                }
                Ok(false) => true,
            }
    ...
    /// Clears all pending solutions from the pool.
    pub fn clear_all_pending_solutions(&self) {
