...
impl<N: Network> FromBytes for CompactBatchHeader<N> {
    /// Reads the compact batch header from the buffer.
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        ...
        let compact_batch_header =
            Self::from(author, round, timestamp, transmission_ids, previous_certificate_ids, signature)
                .map_err(|e| error(e.to_string()))?;

        // Return the compact batch header.
        match compact_batch_header.batch_id == batch_id {
            true => Ok(compact_batch_header),
...
impl<N: Network> ToBytes for CompactBatchHeader<N> {
    /// Writes the compact batch header to the buffer.
...
mod tests {
    ...
    fn test_bytes() {
        ...
        for expected in crate::compact_batch_header::test_helpers::sample_compact_batch_headers(rng) {
