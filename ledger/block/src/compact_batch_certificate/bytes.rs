...
impl<N: Network> FromBytes for CompactBatchCertificate<N> {
    /// Reads the compact batch certificate from the buffer.
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        ...
        Self::from(certificate_id, compact_batch_header, signatures).map_err(|e| error(e.to_string()))
    }
}

impl<N: Network> ToBytes for CompactBatchCertificate<N> {
    /// Writes the compact batch certificate to the buffer.
    fn write_le<W: Write>(&self, mut writer: W) -> IoResult<()> {
        // Write the version.
        0u8.write_le(&mut writer)?;
        // Write the certificate ID.
        self.certificate_id.write_le(&mut writer)?;
        // Write the compact batch header.
        self.compact_batch_header.write_le(&mut writer)?;
...
mod tests {
    ...
    fn test_bytes() {
        ...
        for expected in crate::compact_batch_certificate::test_helpers::sample_compact_batch_certificates(rng) {
            ...
            assert_eq!(expected, CompactBatchCertificate::read_le(&expected_bytes[..]).unwrap());
            assert!(CompactBatchCertificate::<CurrentNetwork>::read_le(&expected_bytes[1..]).is_err());
        }
    }
}
