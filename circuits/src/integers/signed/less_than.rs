...
impl<E: Environment, I, const SIZE: usize> LessThan<Self> for Signed<E, I, SIZE> {
    ...
    fn is_lt(&self, other: &Self) -> Self::Output {
        ...
        for (i, (self_bit, other_bit)) in self.bits_le.iter().zip(other.bits_le.iter()).rev().enumerate() {
            let less_than = if i == 0 {
                self_bit.and(&!other_bit)
            } else {
                (!self_bit).and(other_bit)
            };
            result = result.or(&prev_bits_equal.and(&less_than));
