...
mod tests {
    ...
    fn run_test<I: IntegerType + BitOr<Output = I>>(
        ...
        let check_bitor = | name: &str, first: I, second: I | {
            let a = Integer::<Circuit, I>::new(mode_a, first);
            let b = Integer::<Circuit, I>::new(mode_b, second);
            let case = format!("BitOr: ({} | {})", first, second);
            let expected = first | second;
            check_binary_operation_passes(name, &case, expected, &a, &b, | a: &Integer<Circuit, I>, b: &Integer<Circuit, I> | { a.bitor(b) }, num_constants, num_public, num_private, num_constraints);
