...
mod tests {
    ...
    fn run_test(mode_a: Mode, mode_b: Mode) {
        for i in 0..ITERATIONS {
            ...
            let expected = (<Circuit as Environment>::Affine::zero().to_projective() - second.to_projective()).into();
            check_sub(&name, &expected, &zero, &b);
            let name = format!("SubAssign: 0 - b {}", i);
            check_sub_assign(&name, &expected, &zero, &b);
        }
    }
