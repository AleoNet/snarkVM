...
impl<F: PrimeField, MM: MarlinMode> AHPForR1CS<F, MM> {
    /// Output the number of oracles sent by the prover in the fifth round.
    pub fn num_fifth_round_oracles() -> usize {
        3
    }

    /// Output the degree bounds of oracles in the fifth round.
    pub fn fifth_round_polynomial_info(info: &CircuitInfo<F>) -> BTreeMap<PolynomialLabel, PolynomialInfo> {
    ...
    pub fn prover_fifth_round<'a, R: RngCore>(
        verifier_message: &verifier::FourthMessage<F>,
        mut state: prover::State<'a, F, MM>,
        _r: &mut R,
    ) -> Result<(prover::FifthMessage<F>, prover::FifthOracles<F>, prover::State<'a, F, MM>), AHPError> {
        ...
        let msg = prover::FifthMessage { sum_a, sum_b, sum_c };
        let oracles = prover::FifthOracles { g_a, g_b, g_c };
        state.lhs_polynomials = Some([lhs_a, lhs_b, lhs_c]);
        state.sums = Some([sum_a, sum_b, sum_c]);
        assert!(oracles.matches_info(&Self::fifth_round_polynomial_info(&state.index.index_info)));
