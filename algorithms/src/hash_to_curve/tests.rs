...
mod bls12_377 {
    ...
    fn hash_bls12_377_g1() {
        let g1 = hash_to_curve::<G1Affine, 64>("Aleo BLS12-377 G1 in 0").unwrap();
        assert!(g1.is_on_curve());
        assert!(g1.is_in_correct_subgroup_assuming_on_curve());
        assert_eq!(G1Affine::SERIALIZED_SIZE, 384 / 8);
        assert_eq!(
            try_hash_to_curve::<G1Affine, 64>("Aleo BLS12-377 G1"),
        ...
        assert_eq!(
            ...
            BigInteger384::new([
                1486720206802979903,
                4303701464618689401,
                15665783686427441963,
                2102940834403247893,
                12678861252317921062,
                84563569963191510
            ])
        );
        assert_eq!(
            g1.y.to_repr(),
            BigInteger384::new([
                13359962532070374977,
                8419017305074086619,
                9703847967243099971,
                14233224892165100988,
                4990536703978168266,
                49147409834838302
            ])
        );
