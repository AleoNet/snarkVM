...
mod tests {
    ...
    fn test_size() {
        {
            ...
            assert_eq!(2436, transition_bytes.len(),);
        }
        {
            let transaction = Testnet2::genesis_block().to_coinbase_transaction().unwrap();
            let transition = transaction.transitions().first().unwrap().clone();
            let transition_bytes = transition.to_bytes_le().unwrap();
            assert_eq!(502782, transition_bytes.len(),);
        }
    }
    ...
    fn test_transition_serde_json() {
        ...
        assert_eq!(1005754, candidate_string.len(), "Update me if serialization has changed");
    ...
    fn test_transition_bincode() {
        ...
        assert_eq!(502782, expected_bytes.len(), "Update me if serialization has changed");
