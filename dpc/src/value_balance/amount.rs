...
impl Amount {
    ...
    pub fn add(self, b: Self) -> Result<Self> {
        self.0.checked_add(b.0).ok_or_else(|| anyhow!("Amount addition overflow.")).map(Self::from_gate)
    }

    /// Subtract the value of two `Amount`
    #[allow(clippy::should_implement_trait)]
    pub fn sub(self, b: Self) -> Result<Self> {
        self.0.checked_sub(b.0).ok_or_else(|| anyhow!("Amount subtraction underflow.")).map(Self::from_gate)
    }
