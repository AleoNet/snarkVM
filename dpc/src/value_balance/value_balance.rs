...
use crate::{error::ValueBalanceCommitmentError, Amount, Network};
...
impl<N: Network> ValueBalanceCommitment<N> {
    ...
    fn commit_without_randomness(value: Amount) -> Result<N::ProgramAffineCurve, ValueBalanceCommitmentError> {
...
pub(crate) mod tests {
    ...
    pub(crate) fn generate_random_value_balance_commitment<N: Network, R: Rng>(
        ...
        sighash: &[u8],
        rng: &mut R,
    ) -> (Vec<N::ValueCommitment>, Vec<N::ValueCommitment>, Amount, ValueBalanceCommitment<N>) {
    ...
    fn test_value_balance_commitment() {
        ...
        let input_amount = Amount::from_gate(rng.gen_range(0..100000000));
        let input_amount_2 = Amount::from_gate(rng.gen_range(0..100000000));
        let output_amount = Amount::from_gate(rng.gen_range(0..100000000));
        let output_amount_2 = Amount::from_gate(rng.gen_range(0..100000000));
