...
fn record_decrypt_with_payload(c: &mut Criterion) {
    ...
    let ciphertext_with_payload =
        Record::new(address, Amount::from_gate(1234), Some(Payload::from(&payload)), None, rng)
            .unwrap()
            .ciphertext()
            .clone();
...
fn record_decrypt_with_program_id(c: &mut Criterion) {
    ...
    let ciphertext_with_program_id =
        Record::new(address, Amount::from_gate(1234), None, Some(program_id), rng).unwrap().ciphertext().clone();
...
fn record_decrypt_with_payload_and_program_id(c: &mut Criterion) {
    ...
    let ciphertext_with_program_id_and_payload =
        Record::new(address, Amount::from_gate(1234), Some(Payload::from(&payload)), Some(program_id), rng)
            .unwrap()
            .ciphertext()
            .clone();
