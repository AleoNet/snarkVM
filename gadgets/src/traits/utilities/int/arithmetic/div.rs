    ...
    ($($gadget:ident),*) => ($(
        impl Div for $gadget {
            ...
            fn div<F: PrimeField, CS: ConstraintSystem<F>>(
                ...
                if other.eq(&Self::constant(0 as <$gadget as Number>::IntegerType)) {
                ...
                let allocated_one = Self::alloc(&mut cs.ns(|| "one"), || Ok(1 as <$gadget as Number>::IntegerType))?;
                let one = Self::conditionally_select(
                    &mut cs.ns(|| "constant_or_allocated_1"),
                    &is_constant,
                    &Self::constant(1 as <$gadget as Number>::IntegerType),
                    &allocated_one,
                )?;

                let allocated_zero = Self::alloc(&mut cs.ns(|| "zero"), || Ok(0 as <$gadget as Number>::IntegerType))?;
                let zero = Self::conditionally_select(
                    &mut cs.ns(|| "constant_or_allocated_0"),
                    &is_constant,
                    &Self::constant(0 as <$gadget as Number>::IntegerType),
                    &allocated_zero,
                )?;

                // if the numerator is 0, return 0
                let self_is_zero = Boolean::Constant(self.eq(&Self::constant(0 as <$gadget as Number>::IntegerType)));

                // if other is the minimum number, the result will be zero or one
                // -128 / -128 = 1
                // x / -128 = 0 fractional result rounds to 0
                let min = Self::constant(<$gadget as Number>::IntegerType::MIN);
                ...
                let mut index = <$gadget as Number>::SIZE - 1 as usize;
                let mut bit_value = (1 as <$gadget as Number>::IntegerType) << ((index - 1) as <$gadget as Number>::IntegerType);
