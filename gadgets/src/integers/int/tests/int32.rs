...
#[test]
fn test_int32_to_bits_be() {
    let mut rng = XorShiftRng::seed_from_u64(1231275789u64);

    for _ in 0..1000 {
        let mut cs = TestConstraintSystem::<Fr>::new();

        let byte_val: i32 = rng.gen();
        let byte = Int32::alloc(cs.ns(|| "alloc value"), || Ok(byte_val)).unwrap();

        let bits = byte
            .to_bits_be(cs.ns(|| "to_bits_be"))
            .expect("failed to get i32 bits be");
...
fn test_int32_to_bytes_le() {
    ...
    for _ in 0..1000 {
        ...
        let bytes_from_gadget = byte
            ...
            .expect("failed to get i32 bits le");
...
fn test_int32_from_bits_be() {
    ...
    for _ in 0..1000 {
        ...
        let b = Int32::from_bits_be(&v, cs.ns(|| "from_bits_be")).expect("failed to create Int32 from bits.");
...
fn test_int32_from_bits_le() {
    ...
    for _ in 0..1000 {
        ...
        let b = Int32::from_bits_le(&v, cs.ns(|| "from_bits_le")).expect("failed to create Int32 from bits.");
...
fn test_int32_from_bytes_be() {
    ...
    for _ in 0..1000 {
        ...
        let b = Int32::from_bytes_be(v.clone(), cs.ns(|| "from_bytes_be_gadget"))
            .expect("failed to create a Int32 from a byte");
...
fn test_int32_from_bytes_le() {
    ...
    for _ in 0..1000 {
        ...
        let b = Int32::from_bytes_le(v.clone(), cs.ns(|| "from_bytes_le_gadget"))
            .expect("failed to create a Int32 from a byte");
...
fn test_int32_to_bits_full() {
    ...
    let mut bits_be = byte
        ...
        .expect("failed to get i32 bits be");
    let i32_int_from_be =
        Int32::from_bits_be(&bits_be, cs.ns(|| "from_bits_be")).expect("failed to get i32 from bits be");

    let bits_le = byte
        .to_bits_le(cs.ns(|| "to_bits_le"))
        .expect("failed to get i32 bits le");
    let i32_int_from_le =
        Int32::from_bits_le(&bits_le, cs.ns(|| "from_bits_le")).expect("failed to get i32 from bits le");

    bits_be.reverse();
    assert_eq!(bits_be, bits_le);
    assert_eq!(byte, i32_int_from_be);
    assert_eq!(byte, i32_int_from_le);
    assert!(!cs.is_satisfied());
}

#[test]
fn test_int32_to_bytes_full() {
    let mut cs = TestConstraintSystem::<Fr>::new();
    let byte_val = 0b01110001;
    let byte = Int32::alloc(cs.ns(|| "alloc value"), || Ok(byte_val)).unwrap();
    ...
    let mut bytes_be = byte
        .to_bytes_be(cs.ns(|| "to_bytes_be"))
        .expect("failed to get i32 bytes be");
    let i32_int_from_be =
        Int32::from_bytes_be(bytes_be, cs.ns(|| "from_bytes_be")).expect("failed to get i32 from bytes be");

    let bytes_le = byte
        .to_bytes_le(cs.ns(|| "to_bits_le"))
        .expect("failed to get i32 bytes le");
    let i32_int_from_le =
        Int32::from_bytes_le(bytes_le, cs.ns(|| "from_bytes_le")).expect("failed to get i32 from bytes le");
...
fn test_int32_pow_constants() {
    ...
    for _ in 0..10 {
        ...
        let expected = match a.checked_pow(b as i32) {
            Some(valid) => valid,
            None => continue,
        };
