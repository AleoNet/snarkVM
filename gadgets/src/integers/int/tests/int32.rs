...
fn test_int32_from_bits_be() {
    ...
    for _ in 0..1000 {
        ...
        let b = Int32::from_bits_be(
            v.clone().try_into().expect("failed to convert bits to array"),
            cs.ns(|| "from_bits_be"),
        )
        .expect("failed to create Int32 from bits.");
...
fn test_int32_to_bits_full() {
    ...
    let mut bits_be = byte
        .to_bits_be(cs.ns(|| "to_bits_be"))
        .expect("failed to get i32 bits be");
    let i32_int_from_be = Int32::from_bits_be(
        bits_be.clone().try_into().expect("failed to convert bits to array"),
        cs.ns(|| "from_bits_be"),
    )
    .expect("failed to get i32 from bits be");

    let bits_le = byte
        .to_bits_le(cs.ns(|| "to_bits_le"))
        .expect("failed to get i32 bits le");
    let i32_int_from_le = Int32::from_bits_le(
        bits_le.clone().try_into().expect("failed to convert bits to array"),
        cs.ns(|| "from_bits_le"),
    )
    .expect("failed to get i32 from bits le");
