...
fn test_uint128_from_bytes_be() {
    ...
    for _ in 0..1000 {
        ...
        let v: [UInt8; 16] = expected.to_be_bytes().map(|byte| UInt8::constant(byte));

        let mut cs = TestConstraintSystem::<Fr>::new();

        let b = UInt128::from_bytes_be(v.clone(), cs.ns(|| "from_bytes_gadget"))
...
fn test_uint128_from_bytes_le() {
    ...
    for _ in 0..1000 {
        let expected: u128 = rng.gen();
        let v: [UInt8; 16] = expected.to_le_bytes().map(|byte| UInt8::constant(byte));

        let mut cs = TestConstraintSystem::<Fr>::new();

        let b = UInt128::from_bytes_le(v.clone(), cs.ns(|| "from_bytes_gadget"))
            .expect("failed to create a UInt128 from a byte");
