...
fn test_uint32_to_bits_le() {
    ...
    let bits = byte.to_bits_le(cs).expect("failed to get u32 bits le");
...
fn test_uint32_to_bits_be() {
    ...
    let byte_val = 0b01110001;
    let byte = UInt32::alloc(cs.ns(|| "alloc value"), || Ok(byte_val)).unwrap();
    let bits = byte.to_bits_be(cs).expect("failed to get u32 bits be");
...
fn test_uint32_from_bits() {
    ...
    for _ in 0..1000 {
        ...
        let b = UInt32::from_bits_le(&v, cs).expect("failed to create UInt32 from bits.");
...
fn test_uint32_from_bytes_be() {
    ...
    for _ in 0..1000 {
        ...
        let v: [UInt8; 4] = expected.to_be_bytes().map(|byte| UInt8::constant(byte));

        let mut cs = TestConstraintSystem::<Fr>::new();

        let b = UInt32::from_bytes_be(v.clone(), cs.ns(|| "from_bytes_gadget"))
...
fn test_uint32_from_bytes_le() {
    ...
    for _ in 0..1000 {
        let expected: u32 = rng.gen();
        let v: [UInt8; 4] = expected.to_le_bytes().map(|byte| UInt8::constant(byte));

        let mut cs = TestConstraintSystem::<Fr>::new();

        let b = UInt32::from_bytes_le(v.clone(), cs.ns(|| "from_bytes_gadget"))
            .expect("failed to create a UInt32 from a byte");
