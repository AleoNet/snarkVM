...
use crate::{
    ...
    integers::uint::{Sub, UInt, UInt32, UInt8},
...
#[test]
fn test_uint32_to_bits_be() {
    let mut rng = XorShiftRng::seed_from_u64(1231275789u64);

    for _ in 0..1000 {
        let mut cs = TestConstraintSystem::<Fr>::new();

        let byte_val: u32 = rng.gen();
        let byte = UInt32::alloc(cs.ns(|| "alloc value"), || Ok(byte_val)).unwrap();

        let bits = byte
            .to_bits_be(cs.ns(|| "to_bits_be"))
            .expect("failed to get u32 bits be");
...
fn test_uint32_to_bytes_be() {
    ...
    for _ in 0..1000 {
        ...
        let bytes_from_gadget = byte
            ...
            .expect("failed to get u32 bits le")
            .iter()
...
fn test_uint32_from_bits_be() {
    ...
    for _ in 0..1000 {
        ...
        let b = UInt32::from_bits_be(&v, cs.ns(|| "from_bits_be")).expect("failed to create UInt32 from bits.");
...
fn test_uint32_from_bits_le() {
    ...
    for _ in 0..1000 {
        ...
        let b = UInt32::from_bits_le(&v, cs.ns(|| "from_bits_le")).expect("failed to create UInt32 from bits.");
...
fn test_uint32_from_bytes_be() {
    ...
    for _ in 0..1000 {
        ...
        let b =
            UInt32::from_bytes_be(&v, cs.ns(|| "from_bytes_be_gadget")).expect("failed to create a UInt32 from a byte");
...
fn test_uint32_from_bytes_le() {
    ...
    for _ in 0..1000 {
        ...
        let b =
            UInt32::from_bytes_le(&v, cs.ns(|| "from_bytes_le_gadget")).expect("failed to create a UInt32 from a byte");
...
fn test_uint32_to_bits_full() {
    ...
    let mut bits_be = byte
        ...
        .expect("failed to get u32 bits be");
    let u32_int_from_be =
        UInt32::from_bits_be(&bits_be, cs.ns(|| "from_bits_be")).expect("failed to get u32 from bits be");

    let bits_le = byte
        .to_bits_le(cs.ns(|| "to_bits_le"))
        .expect("failed to get u32 bits le");
    let u32_int_from_le =
        UInt32::from_bits_le(&bits_le, cs.ns(|| "from_bits_le")).expect("failed to get u32 from bits le");

    bits_be.reverse();
    assert_eq!(bits_be, bits_le);
    assert_eq!(byte, u32_int_from_be);
    assert_eq!(byte, u32_int_from_le);
    assert!(cs.is_satisfied());
}

#[test]
fn test_uint32_to_bytes_full() {
    let mut cs = TestConstraintSystem::<Fr>::new();
    let byte_val = 0b01110001;
    let byte = UInt32::alloc(cs.ns(|| "alloc value"), || Ok(byte_val)).unwrap();
    ...
    let mut bytes_be = byte
        .to_bytes_be(cs.ns(|| "to_bytes_be"))
        .expect("failed to get u32 bytes be");
    let u32_int_from_be =
        UInt32::from_bytes_be(&bytes_be, cs.ns(|| "from_bytes_be")).expect("failed to get u32 from bytes be");

    let bytes_le = byte
        .to_bytes_le(cs.ns(|| "to_bits_le"))
        .expect("failed to get u32 bytes le");
    let u32_int_from_le =
        UInt32::from_bytes_le(&bytes_le, cs.ns(|| "from_bytes_le")).expect("failed to get u32 from bytes le");
