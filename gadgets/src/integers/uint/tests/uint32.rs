...
fn test_uint32_from_bits_be() {
    ...
    for _ in 0..1000 {
        ...
        let b = UInt32::from_bits_be(
            v.clone().try_into().expect("failed to convert bits to array"),
            cs.ns(|| "from_bits_be"),
        )
        .expect("failed to create UInt32 from bits.");
...
fn test_uint32_to_bits_full() {
    ...
    let mut bits_be = byte
        .to_bits_be(cs.ns(|| "to_bits_be"))
        .expect("failed to get u32 bits be");
    let u32_int_from_be = UInt32::from_bits_be(
        bits_be.clone().try_into().expect("failed to convert bits to array"),
        cs.ns(|| "from_bits_be"),
    )
    .expect("failed to get u32 from bits be");

    let bits_le = byte
        .to_bits_le(cs.ns(|| "to_bits_le"))
        .expect("failed to get u32 bits le");
    let u32_int_from_le = UInt32::from_bits_le(
        bits_le.clone().try_into().expect("failed to convert bits to array"),
        cs.ns(|| "from_bits_le"),
    )
    .expect("failed to get u32 from bits le");
