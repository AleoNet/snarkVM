...
fn test_uint16_to_bits_le() {
    ...
    let bits = byte.to_bits_le(cs).expect("failed to get u16 bits le");
...
fn test_uint16_to_bits_be() {
    ...
    let byte_val = 0b01110001;
    let byte = UInt16::alloc(cs.ns(|| "alloc value"), || Ok(byte_val)).unwrap();
    let bits = byte.to_bits_be(cs).expect("failed to get u16 bits be");
...
fn test_uint16_from_bits() {
    ...
    for _ in 0..1000 {
        ...
        let b = UInt16::from_bits_le(&v, cs).expect("failed to create UInt16 from bits.");
