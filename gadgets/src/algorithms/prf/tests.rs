...
mod blake2s {
    ...
    fn test_blake2s_prf() {
        ...
        let seed_gadget =
            <Blake2sGadget as PRFGadget<_, Fr>>::Seed::alloc(&mut cs.ns(|| "declare_seed"), || Ok(seed)).unwrap();
        let input_gadget =
            <Blake2sGadget as PRFGadget<_, Fr>>::Input::alloc(&mut cs.ns(|| "declare_input"), || Ok(input)).unwrap();
