...
//
// use crate::{
//     algorithms::{binding_signature::BindingSignatureVerificationGadget, commitment::PedersenCommitmentGadget},
//     boolean::Boolean,
...
// type ValueCommitmentGadget = PedersenCommitmentGadget<G, Fr, EdwardsBls12Gadget, NUM_WINDOWS, WINDOW_SIZE>;
// type VerificationGadget = BindingSignatureVerificationGadget<G, Fr, EdwardsBls12Gadget, NUM_WINDOWS, WINDOW_SIZE>;
//
...
//
//     let binding_commitment_gadget =
//         VerificationGadget::alloc_constant(&mut cs.ns(|| "value_commitment_gadget"), || Ok(value_commitment)).unwrap();
//
//     let c_gadget =
//         <VerificationGadget as BindingSignatureGadget<ValueCommitment, ValueCommitmentGadget, _, _>>::RandomnessGadget
//             > ::alloc(&mut cs.ns(|| "c_gadget"), || Ok(&c)).unwrap();
//
//     let partial_bvk_gadget = <VerificationGadget as BindingSignatureGadget<
//         ValueCommitment,
//         ValueCommitmentGadget,
//         _,
//         _,
...
//
//     let affine_r_gadget =
//         <VerificationGadget as BindingSignatureGadget<ValueCommitment, ValueCommitmentGadget, _, _>>::OutputGadget
//             > ::alloc(&mut cs.ns(|| "affine_r_gadget"), || Ok(affine_r)).unwrap();
//
//     let recommit_gadget =
//         <VerificationGadget as BindingSignatureGadget<ValueCommitment, ValueCommitmentGadget, _, _>>::OutputGadget
//             > ::alloc(&mut cs.ns(|| "recommit_gadget"), || Ok(recommit)).unwrap();
//
...
//
//     let value_balance_comm = binding_commitment_gadget
//         .check_value_balance_commitment_gadget(&mut cs.ns(|| "value_balance_commitment"), &value_balance_bytes)
//         .unwrap();
//
//     <VerificationGadget as BindingSignatureGadget<ValueCommitment, ValueCommitmentGadget, _, G>>::check_binding_signature_gadget(
